...
class nftUtils():
    ...
    def delete_rules(self, table = None, chain = None, comment = None, match = None, family = "ip"):
        ...
        for r in rules:
            ...
            if match is not None and match not in r['expr']:
                matched = False
            if matched:
                self.nft.json_cmd({"nftables": [{ "delete": { "rule": { "family": r['family'], "table": r['table'], "chain": r['chain'], "handle": r['handle'] } } }]})
                n+= 1
    ...
    def delete_table(self, name, handle = None, family="ip"):
        ...
        return self.nft.json_cmd({"nftables": [{ "delete": { "table": {"family": family, "name": name, "handle": handle} } }]})
